...
- **歷史收盤價**：新增合約時，自動抓取交易日收盤價作為 KI / KO / 執行價計算基準
...
| 股價來源 | Yahoo Finance API，經 Netlify Functions 轉發（server 端 proxy） |
...
| KO / KI / 執行價格比例 | 填百分比（如 `100` / `60` / `75`）；KI 填 `0` 時自動使用執行價格比例 |
| 年利率 | 填百分比（如 `21.99` = 年利率 21.99%） |
